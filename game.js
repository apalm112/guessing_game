//My Saturday Work At Fixing The Wrong Answer Text To Red AND No Img Displayed
  ...
  for (var j = 0; j < questions.length; j++) {
    if (qType[j] === '1') {
      if (results[j] === 'o') {
        ...
        //new code below done on Saturday for correct answer
  var image = "img/codefellows.png";
  var sum = document.getElementById('summary1');
  sum.innerHTML = '' + summary + '<br><img style="height:200px; width: 200px;" src="' + image + '">';       // Display the summary results w/ an image
  sum.className = 'fellows'; //Display text in green

      }
      else {
        summary = summary + '<br />In response to Question #' + (j+1) + ': ' + questions[j] + ' you incorrectly answered ' + guesses[j].toLowerCase();

          // New Code below done on Saturday for incorrect answer
  var sum = document.getElementById('summary1');
  sum.innerHTML = '' + summary;       // Display the summary results
  sum.className = 'wrongAnswer';  //Display text in red
      ...
      if (guesses[j].length === 0) {
        summary += ' you correctly guessed ' + answers[j] + ' on the first try!';
                // New Code below done on Saturday
        var image = "img/codefellows.png";
        var sum = document.getElementById('summary1');
        sum.innerHTML = '' + summary + '<br><img style="height:200px; width: 200px;" src="' + image + '">';       // Display the summary results w/ an image
        sum.className = 'fellows';  //Display text in green
...
if (questions.length === 0) {
  ...
  summarize(); //Moved the below code up to the summarize function on line 113 so that the answer text can be green & red appropriately.
    /*if () {
          var image = "img/codefellows.png";
          var sum = document.getElementById('summary1');
          sum.innerHTML = '' + summary + '<br><img style="height:200px; width: 200px;" src="' + image + '">';       // Display the summary results w/ an image
          sum.className = 'fellows';  //Turns summary text green
    } else {
